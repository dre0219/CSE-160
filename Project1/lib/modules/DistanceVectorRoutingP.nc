...
implementation {
    ...
    command void DistanceVectorRouting.routePacket(pack* myMsg) {
        ...
        if(myMsg->dest == TOS_NODE_ID && myMsg->protocol == PROTOCOL_PING) {
            dbg(ROUTING_CHANNEL, "PING reached dest %d\n", TOS_NODE_ID);
            ...
            dbg(ROUTING_CHANNEL, "PING_REPLY reached dest %d\n", TOS_NODE_ID);
            return;
        } else if(myMsg->dest == TOS_NODE_ID && myMsg->protocol == PROTOCOL_TCP) {
            dbg(ROUTING_CHANNEL, "TCP Packet reached dest %d\n", TOS_NODE_ID);
        ...
        if((nextHop = findNextHop(myMsg->dest))) {
            ...
            dbg(ROUTING_CHANNEL, "No route to destination. dropped packet\n");
            //logPack(myMsg);
        }
    }
